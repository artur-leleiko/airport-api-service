...
class AuthenticatedRouteApiTests(TestCase):
    ...
    def test_list_routes(self):
        ...
        destination = sample_airport("Test-2", "Barcelona")
    ...
    def test_filter_routes_by_source(self):
        ...
        source2 = sample_airport("Test-3", "Tokyo")
        destination = sample_airport("Test-2", "Barcelona")
        sample_route(source1, destination, 1000)
        sample_route(source2, destination, 1500)

        res = self.client.get(
            ROUTE_URL, {"source": source1.closest_big_city}
        )

        routes = Route.objects.filter(source=source1)
        serializer = RouteListSerializer(routes, many=True)
    ...
    def test_filter_routes_by_destination(self):
        ...
        destination2 = sample_airport("Test-4", "Berlin")
        sample_route(source, destination1, 1000)
        sample_route(source, destination2, 1500)

        res = self.client.get(
            ROUTE_URL, {"destination": destination1.closest_big_city}
        )

        routes = Route.objects.filter(destination=destination1)
        serializer = RouteListSerializer(routes, many=True)
